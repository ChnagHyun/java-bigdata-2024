...
# desc: PyQt5 앱 만들기 (계속)
'''
PyQt5 ->Qt를 파이썬에서 쓸수 잇도록 만든 라이브러리
...
#QMainWindow, QLabel, QPushButton등은 QWidget을 상속한 자식 클래스(부모 클래스의 노력들을 다 사용할 수 있음)

from PyQt5.QtWidgets import * #QApplication, QWidget, QMainWindow, QLabel

class qtApp(QWidget): #QWidget이 가지고 있는 속성, 변수, 함수를 다 사용가능
    def __init__(self) -> None: 
        super().__init__() # 생성자, 부모클래스의 생성자 함수도 실행되어야 한다.
        self.initUI();

    def initUI(self):    
        label = QLabel() #라벨위젯(qt,PyQt) == 라벨컨트롤(MFC, C#, Java Fx, Anderoid)
